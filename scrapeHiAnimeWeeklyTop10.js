...
export const scrapeHiAnimeWeeklyTop10 = async () => {
    ...
    try {
        ...
        console.log('🔍 Scraping HiAnime weekly top 10 titles...');
        
        // Get weekly top 10 titles from HiAnime
        const hiAnimeWeeklyTitles = await page.evaluate(() => {
            const results = [];
            const processedTitles = new Set();
            
            // Get weekly top anime section
            const weeklySection = document.querySelector('#top-viewed-week.anif-block-ul.anif-block-chart.tab-pane');
            if (weeklySection) {
                console.log('✅ Found weekly section');
                
                // Get top items (rank 1-3)
                const topItems = weeklySection.querySelectorAll('.item-top');
                ...
                const listItems = weeklySection.querySelectorAll('li:not(.item-top)');
            ...
            if (results.length === 0) {
                ...
                const alternativeWeeklySection = document.querySelector('[id*="top-viewed-week"]');
                if (alternativeWeeklySection) {
                    console.log('✅ Found alternative weekly section');
                    
                    const allItems = alternativeWeeklySection.querySelectorAll('li');
        ...
        console.log(`✅ Got ${hiAnimeWeeklyTitles.length} weekly titles from HiAnime:`);
        hiAnimeWeeklyTitles.forEach(item => {
            console.log(`  ${item.rank}. "${item.title}" (${item.category})`);
        });

        // Extract just the titles for metadata fetching
        const titlesList = hiAnimeWeeklyTitles.map(item => item.title);
        ...
        const finalResults = detailedAnimeList.map((anime, index) => ({
            ...anime,
            weekly_rank: hiAnimeWeeklyTitles[index]?.rank || index + 1,
            category: hiAnimeWeeklyTitles[index]?.category || 'unknown'
        }));
        ...
        return [
            {
                ...
                status: "N/A",
                released: "N/A",
                description: `Error occurred while scraping weekly: ${error.message}`,
                weekly_rank: 1,
                category: 'error'
            }
        ];
...
const getDetailedMetadataFrom123Anime = async (titles, browser) => {
    ...
    try {
        ...
        for (let i = 0; i < Math.min(titles.length, 10); i++) {
            ...
            console.log(`\n🔍 [${i + 1}/${titles.length}] Matching weekly title: "${hiAnimeTitle}"`);
            ...
            if (bestMatch) {
                ...
                matchedAnime.push({
                    ...
                    description: detailedMetadata.description || `${hiAnimeTitle} - Popular weekly anime from HiAnime.`
                });
            } else {
                ...
                if (fallbackAnime) {
                    ...
                    matchedAnime.push({
                        ...
                        status: 'Ongoing',
                        released: '2024',
                        description: `${hiAnimeTitle} - Popular weekly anime from HiAnime.`
                    });
                }
            }
        }
        ...
        return titles.map((title, index) => ({
            ...
            status: 'Ongoing',
            released: '2024',
            description: `${title} - Popular weekly anime from HiAnime.`
        }));
    } finally {
        await page.close();
    }
